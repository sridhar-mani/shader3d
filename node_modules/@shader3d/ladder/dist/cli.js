...
async function initCommand(args) {
    ...
    try {
        ...
        // Skill level
        const level = await select(rl, 'What\'s your shader experience level?', [
        ...
        // Template
        const template = await select(rl, 'Choose a template:', [
        ...
        console.log();
        print(`You're starting at Level ${level}: ${SKILL_LEVELS[parseInt(level)].name}`, 'cyan');
...
async function upgradeCommand() {
    ...
    const avgLevel = Math.round(totalLevel / fileCount);
...
async function findShaderFiles(dir) {
    ...
    async function walk(current) {
        ...
        for (const entry of entries) {
            ...
            else if (entry.isFile() && (entry.name.endsWith('.shader3d.ts') ||
